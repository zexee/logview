...
- **`lv_ui`** — `src/ui/` — ncurses/PDCursesMod TUI: windows, line editor (no longer uses `formw`), input dispatch, background filter + search jobs. Compiles twice when `LV_BUILD_GUI=ON`: once against ncurses/PDCursesMod-wincon (for `lv`), once against PDCursesMod GL backend (for `glv`, gated by `-DLV_USE_PDCURSES`).
...
- **`glv`** (optional, `-DLV_BUILD_GUI=ON`) — `src/gui/` — SDL2 + OpenGL3 window hosting the TUI via PDCursesMod's GL backend, with a Dear ImGui menu bar on top. Built from the same `lv_ui` source via the `lv_ui_gui` library target.
...
- **`AppUi::run()` vs `tick()`**: TUI `lv` calls `run()` (blocking). `glv` calls `start()` once then `tick()` once per SDL frame, because the ImGui menu bar drives the rest of the frame.
- **`pdc_no_swap` patch in `third_party/PDCursesMod/gl/`**: PDCursesMod's GL backend normally calls `SDL_GL_SwapWindow` itself. We patched it (one-line guard in `pdcdisp.c` plus an extern in `pdcgl.h` and definition in `pdcscrn.c`) so `glv` can defer the swap until after ImGui draws. Don't re-vendor PDCursesMod without re-applying the patch.
